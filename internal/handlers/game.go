...
func ConstructorSkip(w http.ResponseWriter, r *http.Request) {
	...
	if !decremented {
		http.Error(w, "no rerolls remaining", http.StatusBadRequest)
		return
	}

	spin, err := game.Spin()
	if err != nil {
		log.Printf("Spin (reroll) error: %v", err)
		http.Error(w, "respin failed", http.StatusInternalServerError)
		return
	}
	if err := db.SaveSpin(r.Context(), id, spin); err != nil {
		log.Printf("SaveSpin (reroll) error for session %s: %v", id, err)
		http.Error(w, "respin failed", http.StatusInternalServerError)
		return
	}
...
// handleSessionError distinguishes not-found from transient DB errors.
